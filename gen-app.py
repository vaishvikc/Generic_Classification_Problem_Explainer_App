...
# remove this if you dont want sample data running for demo
else:
    data = pd.read_csv('Heart_Disease_Prediction.csv')
    st.write('Currently running on sample dataset on Heart Disease')
    st.sidebar.write('Currently running on sample dataset - Heart Disease')
    target_column = 'Target'
    feature_columns = [col for col in data.columns if col != target_column]
    feature_columns =[col for col in feature_columns if "ID" not in col and "id" not in col]
    id_column = [col for col in data.columns if "ID" in col or "id" in col][0]
